...
type EditModalProps = { 
    ...
    onClose: () => void; 
    onSave: (item:Omit<DailyTemp, "dt"> & { dt: string }) => void; 
}; 
                ...
                <div className="w-full max-w-md rounded-xl bg-white shadow-2xl"> 
                    ...
                    <div className="space-y-4 p-6"> 
                        ...
                        <div> 
                            ...
                            value={wind_speed} onChange={(e) => setWindSpeed(Number(e.target.value)) } 
                            className="w-full rounded-md border px-3 py-2 focus:border-blue-500 focus:outline-none" /> 
                        </div> 
                        <div> 
                            <label className="mb-1 block text-sm font-medium"> Cloud cover (%) </label> 
                            <input type="number" min={0} max={100} 
                            value={clouds} onChange={(e) => setClouds(Number(e.target.value)) } 
                            className="w-full rounded-md border px-3 py-2 focus:border-blue-500 focus:outline-none" /> 
                        </div> 
                    </div> 
                    <div className="flex justify-end gap-3 border-t px-6 py-4"> 
                        ...
                        <button onClick={() => onSave({...item,humidity,pressure,wind_speed,clouds}) } 
